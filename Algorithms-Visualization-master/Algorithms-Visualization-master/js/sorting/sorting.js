var algoName = document.getElementById("sort_type").value;
var heading = document.querySelector(".heading");
document.getElementById("sortButton").addEventListener("click", startSort);  

function headerDisplay(algoName) {
  if (algoName == "bubble") {
      heading.innerHTML = "Bubble Sort [O(n)]"
  } else if (algoName == "insertion") {
    heading.innerHTML = "Insertion Sort [O(n)]"    
  } else if (algoName == "selection") {
    heading.innerHTML = "Selection Sort [O(n" +"2".sup()+")]"    
  }
  else if (algoName == "merge") {
    heading.innerHTML = "Merge Sort [O(n log(n))]"    
  }
  else if (algoName == "quick") {
    heading.innerHTML = "Quick Sort [O(n log(n))]"    
  }
}
...
function algoSelected() {
  algoName = document.getElementById("sort_type").value;
...
function generateRandomArray() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    bars[i].style.backgroundColor = 'rgb(0, 0, 128)';
...
async function SortedAnimation() {
  ...
  // then we will again go from left to right and color them blue.
  for (let i = 0; i < n; i++) {
    bars[i].style.backgroundColor = 'rgb(0, 0, 128)';
    await sleep(10);
  }
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = i + 1; j < n; j++) {
      ...
      if (!isPaused) {
        ...
        draw([i, j, minIndex], ['#a6b3ce', '#a1aec9', '#a1aec8']);
      } else {
        j--;
      }
      await sleep(delay);
      ...
      while (i < n1 || j < n2) {
        ...
        if (!isPaused) {
          if (j == n2 || (i < n1 && L[i] <= R[j])) {
            draw([k, ...barsIndices], ['#343434', ...barsColors]);
            i++;
          } else {
...
async function quickSort() {
  ...
  while (!s.isEmpty()) {
    ...
    for (let i1 = l; i1 <= h; i1++) {
      barsIndices.push(i1);
      barsColors.push('rgb(22 131 240)');
    }
    ...
    for (let j = l; j <= h - 1; j++) {
      ...
      if (!isPaused) {
        draw([i, j, ...barsIndices], ['#a6b3ce', '#343434', ...barsColors]);
